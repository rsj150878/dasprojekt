...
public class DBShowNeu {
	...
	public List<ShowRing> getRingeFuerShow(Integer idSchau) throws Exception {
		...
		while (resultSet.next()) {
			...
			zw.addShowKlassen(getKlassenForShow(idSchau, Integer.valueOf(resultSet.getInt("idschauring"))));
			resultList.add(zw);
		}
	...
	public List<ShowKlasse> getKlassenForShow(Integer idSchau, Integer idSchauRing) throws Exception {
		Connection conn = DBConnectionNeu.INSTANCE.getConnection();
		PreparedStatement st = conn
				.prepareStatement("select distinct rasse, klasse from schauhund where idschau = ? and idschauring = ?");
		st.setInt(1, idSchau.intValue());
		...
		while (resultSet.next()) {
			...
			Rassen zwrasse = Rassen.getRasseForKurzBezeichnung(resultSet.getString("rasse"));
