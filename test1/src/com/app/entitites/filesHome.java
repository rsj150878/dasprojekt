...
public class filesHome {
	...
	public files findById(int id) {
		...
		try {
			files instance = (files) sessionFactory.getCurrentSession().get("files", id);
			if (instance == null) {
	...
	public List findByExample(files instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("files").add(Example.create(instance))
					.list();
